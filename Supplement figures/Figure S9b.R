...
######################################## stats and plot
  ...
  run_anova_tukey <- function(response, group, sub1) {
    formula <- as.formula(paste(response, "~", group))
    aov_model <- aov(formula, data = sub1)
    ...
    anova_result <- sprintf(
      "F(%d,%d) = %.2f, p = %.2e",
      anova_df, anova_resid, anova_f, anova_p_value
    )
  
    #get CLDs
    tukey <- HSD.test(aov_model, group, group = TRUE)
    cld <- tukey$groups %>%
  ...
  ########## results
  tukey_results <- cross_df(list(response = response_vars, group = group_vars)) %>%
    mutate(results = map2(response, group, ~ run_anova_tukey(.x, .y, df))) %>%
    unnest(results)
  ...
  plot_annotations <- tukey_results %>%
    ...
    select(Metric, Factor, Level, CL_label, Anova_result) %>%
    distinct()
  ...
  label_data_clean <- data_plot %>%
    ...
    left_join(
      ...
      ungroup() %>%
      mutate(across(c(Level, Metric, Factor), as.character)),
      by = c("Level", "Metric", "Factor")
    )
  
  label_data_pval <- data_plot %>%
    select(Level, Metric, Factor, Anova_result, Value) %>%
    group_by(Metric, Factor) %>%
    summarise(
      p_value_label = shQuote(first(Anova_result)),
...
png("Figure S9b.png", width = 300, height = 300, units = "mm", res = 1000)
